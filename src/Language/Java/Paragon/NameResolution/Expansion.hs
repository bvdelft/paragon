...
type Expansion =
    Map.Map (String, NameType)  -- (Partially) unresolved 
            (Either ContextualError (Maybe Name, NameType)) 
...
mkPkgExpansionWithPrefix :: Maybe Name -> String -> Expansion
...
mkTypeExpansionWithPrefix :: Maybe Name -> String -> Expansion
...
mkExpExpansionWithPrefix :: Maybe Name -> String -> Expansion
...
mkMethodExpansionWithPrefix :: Maybe Name -> String -> Expansion
...
mkLockExpansionWithPrefix :: Maybe Name -> String -> Expansion
...
expansionUnion = foldl (Map.unionWithKey foldC) Map.empty
  where foldC (i,_) res1 res2 = do 
          ...
          (mPre2, _) <- res2
          if (mPre1 /= mPre2)
           then Left $ ambiguousName i mPre1 mPre2 errorAnnotation
           else res1
