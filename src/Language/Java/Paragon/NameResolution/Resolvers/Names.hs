  ...
  case namePrefix n of
    ...
    Nothing  -> do expn <- getExpansion
                   case Map.lookup (idIdent (nameId n), nameType n) expn of
                     ...
                     Nothing       -> do failEC n $ unresolvedName n n  
...
resolveAmbig :: Name -> Resolve Name
resolveAmbig pre n = do
  case nameType pre of
    ...
    LockName   -> failEC n $ illegalDeref "lock" (unparsePrint n) n
    MethodName -> failEC n $ illegalDeref "method" (unparsePrint n) n
...
resolvePkgOrType n = do
  ...
  if isType
   then return tName
     ...
     if isPkg
      then return pName
      else failEC n $ unresolvedName n n
...
resolveCandExp :: Name -> Resolve Name
resolveCandExp pre n = do
  case nameType pre of
    PkgName  -> -- A package cannot contain expressions directly
                failEC n $ exprInPkgError "field, variable or lock" pre n n
    TypeName -> return n
    ExpName  -> return n
...
resolveCandMethod :: Name -> Resolve Name
resolveCandMethod pre n = do
  case nameType pre of
    PkgName  -> -- A package cannot contain methods/locks directly
                failEC n $ exprInPkgError "method or lock" pre n n
    TypeName -> return n
    ExpName  -> return n
    -- other types are not expected
...
resolveLock :: Name -> Resolve Name
resolveLock pre n = do
  case nameType pre of
    PkgName  -> -- A package cannot contain methods/locks directly
                failEC n $ exprInPkgError "lock" pre n n      
