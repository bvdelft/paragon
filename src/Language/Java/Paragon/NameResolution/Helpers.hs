...
-- Fails with error if argument not a type
withTypeCurr :: Name SrcSpan -> NameRes a -> NameRes a
...
checkForType :: Name SrcSpan -> NameRes Bool
checkForType n = do
  n' <- getCurrentName
